#include "main.h"
extern unsigned char reset_flag; //reset flag can be used this file
//array of strings
char *washing_program[] = {"Daily", "Heavy", "Delicates", "Whites", "Stain wash", "Eco cottons",
...
void run_program(unsigned char key) {
    ...
    if (reset_flag == RESET_START_SCREEN) {
        ...
        rinse_time = (unsigned int) total_time * 0.12;
        spin_time = (unsigned int) total_time * 0.42;
        //turn on timer
        TMR2ON = 1;
        FAN = ON;
...
void door_status_check()
{
    if (RB0 == 0)
    {
        ...
        //wait untill switch is released or the door is clsed
        while(RB0 == 0)
        {
